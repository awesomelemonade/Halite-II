...
public class AdvancedStrategy2 implements Strategy {
	...
	public AdvancedStrategy2(GameMap gameMap) {
	...
	public void newTurn() {
		...
		while(!queue.isEmpty()) {
			...
			if(planetRequests.get(targetPlanetId)>0) {
				Group group = micro.getGroup(gameMap.getMyPlayer().getShip(popped));
				DebugLog.log("\tAssigned "+group.getShips().size()+" ships with "+planetRequests.get(targetPlanetId)+" requests");
				int count = 1; //1 for popped ship
				for(int shipId: group.getShips().keySet()) {
					if(queue.contains(shipId)) {
						queue.remove(shipId);
						shipToPlanet.put(shipId, targetPlanetId);
						count++;
					}
				}
				shipToPlanet.put(popped, targetPlanetId);
				planetRequests.put(targetPlanetId, planetRequests.get(targetPlanetId)-count);
			}else {
				...
				for(int shipId: shipToDistance.keySet()) {
					...
					if(pPlanet==parentPlanet&&queue.contains(shipId)) {
				...
				if(index==-1) {
					...
					double distance = gameMap.getMyPlayer().getShip(popped).getPosition()
							.getDistanceTo(newPlanet.getPosition())-newPlanet.getRadius();
					shipToDistance.put(popped, distance);
